...
install.packages("readxl")  # l 아파벳트 엘      , 액셀을 읽어오도록 하는 패키지 
library(readxl)
...
install.packages("readxl")  # 액셀을 읽어오도록 하는 패키지 
library(readxl)

df_exam_sheet <- read_excel("excel_exam_sheet.xlsx", sheet = 3)  # 엑셀안에 sheet중에 3을 불러옴 
View(df_exam_sheet)
...
View(exam) # 엑셀과 유사한 창으로 자료를 보여줌, 대문자 V만 가능 
dim(exam) # 구성 행렬 크기 
str(exam) # 데이터 속성 확인
...
# Rdata 불러오기
rm(df_midterm)                     #df_midterm 변수를 삭제한다.
df_midterm

load("df_midterm.rda")             #rda는 load로 불러와야함.
df_midterm
...
# ---------------
install.packages(dplyr)
library(dplyr)                            #새로 이름을 바꿀 때 쓰는 패키지
df_new <- rename(df_midterm, korean=science) # rename 새로 이름을 붙인다. science를 korean으로 바꿈.
# 변수 타입
...
var2 <- factor(c(30,20,50,20,56))   # factor 변수  Levels 이 붙는다.
var2

var2+3           #숫자가 아니므로 사칙연산이 안됌 
...
#f <- as.factor(),  as.factor()  =>  전자는 f에 저장하는 것이고 , 후자는 그냥 그 형태로 출력만 하는 것
#factor의 경우 as.를 안붙여도 상관x, 나머지는 붙여야 한다.
# 문자로 구성된 factor 변수 
...
# 3. 매트릭스(matrix)
x2 <- matrix(c(1:12), ncol=2)             #ncol은 열이2개  
x2
class(x2)
...
mpg <- ggplot2 :: mpg #::  => 내장 함수가 가지고있는 데이터를 활용한다. ggplot2 안의 내장된 데이터인 mpg를 사용한다.
View(mpg)
class(mpg)
x <- boxplot(mpg$cty)  #mpg안의 cty만 사용하여 boxplot을 저장 , 돌출 => 원으로 표시되는 것들, 굵은 줄이 평균 
x
...
survey <- c(100.023, 101.014, 99.921, 100.011, 100.001, 99.987, 99.999, 99.823, 100.092, 99.876, 100.103)
boxplot(survey)
mean(survey)

# 이상치를 결측값으로 처리하기
survey <- ifelse(survey > 100.5, NA, survey) #ifelse 만약에 그렇다면... survey에서 100.5를 초과하는 변수를 NA로 놓고 새로 survey로 다시만든다. 
survey
mean(survey)

table(is.na(survey)) #is => ~~가 있니    table => 질문에 대한 결과를 트루 폴스를 갯수
...
# 자료를 파악했으면 필요한 자료의 이름을 쉽게 기억할 수 있는 이름(변수)으로 저장 - 변수면 바꾸기
